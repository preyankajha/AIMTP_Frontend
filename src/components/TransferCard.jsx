...
import { MapPin, Building, ArrowRight, Activity, CalendarDays, Trash2, Briefcase, Info, PencilLine, Phone, Lock, ChevronDown } from 'lucide-react';
        ...
        <div className="p-6 flex-1">
          ...
          <div className="bg-[#F8FAFC] rounded-[1.5rem] p-5 border border-slate-100 relative mb-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative">
              ...
              {/* To */}
              <div className="flex-1 min-w-0 w-full text-left sm:text-right relative">
                <div className="flex items-center justify-start sm:justify-end gap-1.5 mb-1">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block px-1">To</span>
                  {transfer.desiredLocations?.length > 1 && (
                    <div className="relative">
                      <button 
                        onClick={(e) => { e.stopPropagation(); setShowMoreLocs(!showMoreLocs); }}
                        className="text-[9px] font-black text-primary-600 bg-primary-100 hover:bg-primary-200 px-2 py-0.5 rounded-md flex items-center gap-1 transition-all border border-primary-200"
                      >
                        +{transfer.desiredLocations.length - 1} MORE <ChevronDown className={`h-2.5 w-2.5 transition-transform ${showMoreLocs ? 'rotate-180' : ''}`} />
                      </button>

                      {/* Centered Modal via Portal */}
                      {showMoreLocs && createPortal(
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                           {/* Backdrop */}
                           <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm animate-fade-in" onClick={(e) => { e.stopPropagation(); setShowMoreLocs(false); }} />
                           
                           {/* Modal Content */}
                           <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl relative z-10 overflow-hidden animate-zoom-in flex flex-col max-h-[70vh]">
                              <div className="px-5 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
                                 <div>
                                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest leading-none">Desired Postings</h3>
                                    <p className="text-[9px] text-slate-400 font-bold mt-1 uppercase tracking-tighter">{transfer.desiredLocations.length} Locations ({transfer.userId?.name.split(' ')[0]})</p>
                                 </div>
                                 <button 
                                    onClick={(e) => { e.stopPropagation(); setShowMoreLocs(false); }}
                                    className="h-7 w-7 bg-white border border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:text-slate-900 transition-all hover:rotate-90"
                                 >
                                    <span className="text-base font-black leading-none">&times;</span>
                                 </button>
                              </div>

                              <div className="p-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 flex-1">
                                 {transfer.desiredLocations.map((loc, i) => (
                                    <div key={i} className="px-4 py-2.5 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors group/loc">
                                       <div className="flex items-center gap-3">
                                          <div className={`p-1.5 rounded-lg shrink-0 border transition-colors ${i === 0 ? 'bg-primary-50 border-primary-100 text-primary-600' : 'bg-emerald-50 border-emerald-100 text-emerald-500'}`}>
                                             <MapPin className="h-3.5 w-3.5" />
                                          </div>
                                          <div className="min-w-0 flex-1">
                                             <div className="flex items-center justify-between gap-2">
                                                <p className="text-[11px] font-black text-slate-900 truncate tracking-tight">{loc.station}</p>
                                                <span className={`shrink-0 text-[8px] font-black px-1.5 py-0.5 rounded-md border ${i === 0 ? 'bg-primary-100/50 text-primary-700 border-primary-200' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                                                   {i === 0 ? 'PRMRY' : `P${loc.priority || i + 1}`}
                                                </span>
                                             </div>
                                             <div className="flex items-center gap-1.5 mt-0.5">
                                               <p className="text-[9px] text-slate-400 font-bold truncate uppercase tracking-tighter">{loc.zone} {zoneCode(loc.zone)}</p>
                                               {loc.division && (
                                                  <>
                                                    <span className="text-slate-200">•</span>
                                                    <p className="text-[9px] text-slate-400 font-bold truncate uppercase tracking-tighter opacity-80">{loc.division}</p>
                                                  </>
                                               )}
                                             </div>
                                          </div>
                              ...
                              <div className="px-5 py-3.5 bg-slate-50 border-t border-slate-100 flex justify-center shrink-0">
                                 <button 
                                    onClick={(e) => { e.stopPropagation(); setShowMoreLocs(false); }}
                                    className="w-full py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black text-slate-500 hover:bg-slate-50 active:scale-95 transition-all shadow-sm uppercase tracking-widest"
                                 >
                                    Close List
                                 </button>
                              </div>
